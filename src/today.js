...
function createTodayTab() {
    ...
    for (let i = 0; i < logic.tasks.length; i++) {
        // check if date of task is today
        if (isToday(parseISO(logic.tasks[i].date))){
    ...
    todayContainer.append(title, displayTasksTable)
    contentDiv.appendChild(todayContainer)
}
export {createTodayTab}